			...
			if(this.state.firstnameText){
				if(this.state.lastnameText){
					if(this.state.passwordText){
						if(this.state.passwordText == this.state.confPasswordText){
							if(this.state.emailText){
								var emailValidate = /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/;
								if(this.state.emailText.match(emailValidate)){
									if(this.state.contactText){
										var phoneno = /^\d{10}$/;
										if(this.state.contactText.match(phoneno)){
											if(this.state.type_id){
												if(this.state.zone_id){
													if(this.state.street_id){
														if(this.state.houseNameText){
															if(this.state.pincodeText){
																var pincode = /^\d{6}$/;
																if(this.state.pincodeText.match(pincode)){
																	if(this.state.avatarSource){
																		...
																		Alert.alert('', 'Avatar is not selected!');
																	}
																}else{
																	Alert.alert('', 'pincode is not valid!');
																}
															}else{
																Alert.alert('', 'pincode is empty!');
															}
														}else{
															Alert.alert('', 'Housename is empty!');
														}
													}else{
														Alert.alert('', 'street is not selected!');
													}
												}else{
													Alert.alert('', 'zone is not selected!');
												}
											}else{
												Alert.alert('', 'Type of house is not selected!');
											}
										}else{
											Alert.alert('', 'Contact no. is not valid!');
										}
									}else{
										Alert.alert('', 'Contact no. is empty!');
									}
								}else{
		...
		.then((responseData) => {
			if(responseData.code == 200){
				this.setState({
					...
					mainCoupon: this.state.couponCode
				})
	...
	createUser(address_id){
		...
		.then((responseData) => {
			if(responseData === null){
				...
				if(this.state.mainCoupon != ''){
					this.sendCoupon(responseData);
...
sendCoupon(id){
	fetch(url.other + "updateUsedCode", {
		method: "POST",
		headers: {
			'Accept': 'application/json',
			'Content-Type': 'application/json'
	...
	getBonus(id){
		...
		.then((responseData) => {
			if(responseData.code == 200){
				Alert.alert('', 'Congratulations! you have earned 500 points as early registration bonus!');
		...
		return(
			<View style={{flex:1,backgroundColor:'#fff', flexDirection:'column'}}>
				...
				<View style = {{flex:1}} >
					...
					<View style = {{flexDirection:'row',height: responsiveHeight(6), width: responsiveWidth(88),backgroundColor:'transparent', borderRadius: 3,marginTop: 10, alignSelf:'center', borderWidth: 1, borderColor:'#7A7A7A' }} >
				    	<View style = {{width: responsiveWidth(9), justifyContent:'center', alignItems:'center',backgroundColor:'#46b07b'}}>
				    		<SimpleLineIcons name = "lock-open" color = "#fff" size = {15} style = {{}}/>
						</View>
						<TextInput
				            ...
				            placeholderTextColor = "#000"
				            underlineColorAndroid = 'transparent'
				            onChangeText={e => this.passwordField(e)}
				            editable={true}
					...
					<View style = {{flexDirection:'row',height: responsiveHeight(6), width: responsiveWidth(88),backgroundColor:'transparent', borderRadius: 3,marginTop: 10, alignSelf:'center', borderWidth: 1, borderColor:'#7A7A7A' }} >
				    	<View style = {{width: responsiveWidth(9), justifyContent:'center', alignItems:'center', backgroundColor:'#46b07b'}}>
				    		<SimpleLineIcons name = "lock" color = "#fff" size = {15} style = {{}}/>
						</View>
						<TextInput
				            value= {this.state.confPasswordText}
				            style={{ color: '#000', height: responsiveHeight(6), width: responsiveWidth(79) , backgroundColor:'transparent', borderRadius: 3, fontSize: responsiveFontSize(1.7), fontFamily: 'Muli-Light',  }}
				            placeholder= {registration.confPasswordPlaceholder}
				            placeholderTextColor = "#000"
				            underlineColorAndroid = 'transparent'
				            onChangeText={e => this.confPasswordField(e)}
				            editable={true}
				            secureTextEntry = {true}
				          />
					</View>
	                 ...
	                 <View style = {{flexDirection: 'row', width: responsiveWidth(88), alignSelf:'center', height: responsiveHeight(7.2), marginTop: 10}}>
    					...
    					<View style = {{width: responsiveWidth(60), height: responsiveHeight(7.2)}}>
    		                <Autocomplete
					          ...
					          renderItem={({ name,id }) => (
					            <TouchableOpacity onPress={() => {this.setState({ query2: name,zone_id:id }); this.select_zone(id);}}>
					              <Text>
					                {name}
					              </Text>
					            </TouchableOpacity>
	               ...
	               <View style = {{flexDirection: 'row', width: responsiveWidth(88), alignSelf:'center', height: responsiveHeight(7.2), marginTop: 12}}>
	                ...
	                {!this.state.showApartmentDrop && <View style = {{flexDirection:'row',height: responsiveHeight(6), width: responsiveWidth(88),backgroundColor:'transparent', borderRadius: 3,marginTop: 15, alignSelf:'center', borderWidth: 1, borderColor:'#7A7A7A' }} >
	                				    	<View style = {{width: responsiveWidth(9), justifyContent:'center', alignItems:'center', backgroundColor:'#46b07b'}}>
	                ...
	                {this.state.showApartmentDrop && <View style = {{flexDirection: 'row', width: responsiveWidth(88), alignSelf:'center', height: responsiveHeight(6), marginTop: 17,}}>
					...
					{this.state.couponSuccess === true && <View style = {{width: responsiveWidth(88), alignSelf:'center', marginBottom: 5}}>
					    		        	<Text style = {{color:'green', fontFamily:'Muli-Bold', fontSize: responsiveFontSize(1.7)}}>Congratulations!  {this.state.codePoint} points added to your account.</Text>
					    		        </View>}
					...
					<TouchableOpacity disabled = {this.state.buttonDisable} onPress = {() => this.submitDetails()} style = {{width: responsiveWidth(88), height: responsiveHeight(6), backgroundColor:'#46b07b', alignSelf:'center', marginBottom:10, borderRadius: 3, justifyContent:'center', marginTop: 10}} >
				...
				<PopupDialog
            		...
            		<View style = {{width: responsiveWidth(85), height: responsiveHeight(40)}}>
            			<TouchableOpacity onPress = {() => this.popupDialog.dismiss()}  style = {{position: 'absolute',width: responsiveWidth(5), height: responsiveHeight(3), backgroundColor:'#46b07b', zIndex: 999, borderRadius: 50, top: 0, right: 0,alignItems:'center', justifyContent:'center'}}>
