...
import { ranking } from '../utils/internationalisation';
...
export default class Ranking extends Component {
	...
	static navigationOptions = {
       ...
       drawerLabel: "Ranking",
 	 	...
 	 	state = {
	        ...
	        filtervalue :'',
	        rankingHeading:'Overall Ranking',
	        shootNum : 0,
	        ssImage: '',
	        zone_locality:'',
 	 	}
    ...
    usefilter(){
    	...
    	if(this.state.filtervalue === 0){
    		filter = 'overall';
    		this.setState({
    			rankingHeading: ranking.mainHeading
    		})
    	}else if(this.state.filtervalue === 1){
    		filter = 'zone';
    		/*this.setState({
    			rankingHeading: ranking.mainHeading2
    		})*/
    	}else if( this.state.filtervalue === 2){
    		filter = 'street';
    		/*this.setState({
    			rankingHeading: ranking.mainHeading3
    		})*/
    	}
    	//alert(filter)
    	fetch(url.other + "rank", {
        ...
        body: JSON.stringify({
          ...
          .then((responseData) => {
          	if(filter!='overall')
            this.setState({
            	rankingHeading: responseData.name + '  Ranking'
            })
        else{
           this.setState({
            	rankingHeading: 'Overall Ranking'
    ...
    shareSs(image){
		...
		Share.open(shareImageBase64).catch((err) => { err && alert.log(err); })
    }
			...
			<View style={{flex:1, backgroundColor:'#eee'}}>
				<View style={Css.header_main}>
					...
					<View style = {Css.header_main_text_view}>
						<Text style ={Css.header_main_text}>{ranking.headerTitle}
						</Text>
					</View>
				</View>
					...
					<View style = {{width: responsiveWidth(100), alignItems:'center', justifyContent:'center'}}>
						<TouchableOpacity style = {{flexDirection:'row', width:responsiveWidth(49.9)}} onPress = {() => this.filterPopupDialog.show()} >
							<View style = {{justifyContent:'center',alignItems:'flex-end', width: responsiveWidth(40)}}>
								<Text style = {{color: '#fff', fontSize: responsiveFontSize(2.8), fontFamily:'Muli-Bold', paddingRight: 7}} >{ranking.filterText}</Text>
							</View>
				...
				<View style = {{flex: 1, flexDirection: 'column'}} >
					<ScrollView>
						...
						<View style = {{alignSelf:'center', width: responsiveWidth(88), height: responsiveHeight(9), backgroundColor:'#fff', marginTop: 20, flexDirection:'row'}} >
							<View style = {{width: responsiveWidth(14), alignItems:'center', justifyContent:'center'}}>
								<Text style = {{fontFamily:'Muli-Bold', fontSize: responsiveFontSize(2.3), color:'#46b07b'}} >{this.state.user_rank.rank}</Text>
							</View>
							<View style = {{width: responsiveWidth(36.3), justifyContent:'flex-start', alignItems:'center', flexDirection:'row'}} >
								<Image source = {{uri: this.state.user_rank.profile_pic }} style = {styles.ProfileImage}/>
								<View style = {{width: responsiveWidth(32.4)}}>
									<Text numberOfLines={1} style = {{fontFamily: 'Muli-Bold', fontSize: responsiveFontSize(2.3), color:'#000', paddingLeft: 20}}>{this.state.user_rank.username}</Text>
								</View>
							</View>
						...
						<View style = {{alignItems:'center', marginTop: 20, marginBottom: 20}} >
							<Text style = {{color:'#46b07b', fontSize: responsiveFontSize(3), fontFamily:'Muli-ExtraBold'}}>{ranking.topTenText}</Text>
						</View>
						<ListView
				...
				<PopupDialog
            		...
            		<View style = {{width: responsiveWidth(85), height: responsiveHeight(45)}}>
            			...
            			<View style = {{width: responsiveWidth(85), height:responsiveHeight(8),borderBottomWidth:2, borderColor:'#46b07b', alignItems:'center', justifyContent:'center'}} >
            				<Text style = {{color: '#46b07b', fontSize: responsiveFontSize(2.8), fontFamily:'Muli-ExtraBold'}} >{ranking.popupHeading}</Text>
            			</View>
            			<View style = {{flexDirection:'column'}} >
            				<View style = {{alignItems:'center'}}>
            					<Text style = {{color: '#46b07b', fontFamily:'Muli-ExtraBold', fontSize:responsiveFontSize(2.4), paddingTop: 5}} >{ranking.popupFilterHead}</Text>
            				</View>
            				<RadioForm
				            ...
				            <View style = {{alignItems: 'center', height: responsiveHeight(8), width: responsiveWidth(85),marginTop: 10}} >    
					            <TouchableOpacity onPress = {() => this.usefilter()} style = {{height: responsiveHeight(6), width: responsiveWidth(30), backgroundColor:"#46b07b", alignItems:'center', justifyContent:'center'}} >
					            	<Text style = {{color:'#fff', fontSize: responsiveFontSize(2.4), fontFamily:'Muli-ExtraBold'}} >{ranking.applyButtonText}</Text>
					            </TouchableOpacity>
				            </View>
            			</View>
            		</View>	
	  ...
	  renderranking(rank){
	  	...
	  	return(
	  		<View style = {{alignSelf:'center', width: responsiveWidth(88), height: responsiveHeight(9), backgroundColor:'#fff', marginBottom: 15, flexDirection:'row'}} >
							...
							<View style = {{width: responsiveWidth(51), justifyContent:'flex-start', alignItems:'center', flexDirection:'row'}} >
								<Image source = {{uri: rank.profile_pic}} style = {styles.ProfileImage}/>
								<View style = {{width: responsiveWidth(42)}}>
									<Text numberOfLines={1} style = {{fontFamily: 'Muli-Bold', fontSize: responsiveFontSize(2.3), color:'#000', paddingLeft: 20}}>{rank.username}</Text>
								</View>
							</View>
